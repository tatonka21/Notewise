...
export default function AIScreen() {
  ...
  const executeActions = (actions: AiAction[]): NoteItem[] => {
    ...
    for (const act of actions) {
      ...
      } else if (act.action === "generate_app") {
        addApp({
          type: "app",
          name: act.title ?? "Generated App",
          description: act.content ?? "",
          code: act.content ?? "",
        });
      } else if (act.action === "generate_website") {
        addApp({
          type: "website",
          name: act.title ?? "Generated Website",
          description: act.content ?? "",
          code: act.content ?? "",
          preview: act.content, // HTML code is the preview
        });
      }
    }
    return created;
  ...
  const handleGenerateApp = async (description: string) => {
    setIsLoading(true);
    try {
      ...
      if (result.success && result.app) {
        const appCode = JSON.stringify(result.app, null, 2);
        addApp({
          type: "app",
          name: result.app.appName || "Generated App",
          description,
          code: appCode,
        });
        const assistantMsg: Message = {
          ...
          role: "assistant",
          content: `I've generated a React Native app called "${result.app.appName}". You can preview and build it now!`,
        };
  ...
  const handleGenerateWebsite = async (description: string) => {
    ...
    try {
      ...
      if (result.success && result.website) {
        addApp({
          type: "website",
          name: result.website.siteName || "Generated Website",
          description,
        ...
        const assistantMsg: Message = {
          id: Date.now().toString(),
          role: "assistant",
          content: `I've generated a website called "${result.website.siteName}". You can preview it now!`,
        };
        setMessages((prev) => [...prev, assistantMsg]);
      }
    } catch (err) {
